...
if(isset($_GET["del"]))
{
    if($_GET["del"]=="success")
    {
        echo "<p class'success-text' style='color:green; font-size:20px;text-align: center;'>Conversation Deleted!<br> </p>";
    }
    else if($_GET["del"]=="failed")
    {
    echo "<p class'success-text' style='color:red; font-size:20px; text-align: center;'>Error Deleting Conversation!<br></p>";
    }
}
if(isset($_GET["selfmsg"]))
{
    if($_GET["selfmsg"]=="true")
    {
       echo "<p class'success-text' style='color:blue; font-size:20px;text-align: center;'>This is You , You can't message yourself!<br> </p>"; 
    }
}
?>
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>messages chat with tabs - </title>
                            ...
                            <div class="contacts-outter" >
                                <ul class="list-unstyled contacts" >
                                    ...
                                    if(isset($_SESSION) && isset($_SERVER['HTTP_REFERER']))
                                    {
                                        ...
                                        $sql="SELECT sender_id,receiver_id,chat_id from chats where sender_id=$user_id or receiver_id=$user_id AND chat_id=(select mchat_id from chat_messages ORDER by msg_date Desc LIMIT 1)";
                                        $result = $conn->query($sql);
                                        if ($result->num_rows > 0)
                                        {
                                        while($row = $result->fetch_assoc())    //start while loop
                                        {
                                    // echo $row["sender_id"]."<br>";
                                    $receiver_id=$row["receiver_id"];
                                    if($row["sender_id"]!=$user_id)
                                    {
                                    $cid=$row["sender_id"];
                                    $sql2="SELECT fname,lname,gender from account where accountid=$cid";
                                    ...
                                    $sql1="SELECT fname,lname,gender from account where accountid=$cid";
                                    ...
                                    {
                                    $cid=$user_id;
                                    $sql3="SELECT fname,lname,gender from account where accountid=$user_id";
                                    ...
                                    $chat_id=$row["chat_id"];
                                    $sql4="select msg_date from chat_messages where mchat_id=$chat_id order by msg_date ASC LIMIT 1";
                                    $result4 = $conn->query($sql4);
                                    if($result4->num_rows > 0)
                                    {
                                    $row4 = $result4->fetch_assoc();
                                    $chat_date=$row4["msg_date"];
                                    }
                                    $date = date('Y-m-d',strtotime($chat_date));
                                    $time = date('h:i:s',strtotime($chat_date));
                        ?>
                                    <li data-toggle="" data-target="#inbox-message-1" class="active">
                                        <!-- <div class="message-count"></div> -->
                                        <img alt="" class="img-circle medium-image" src="img/chat.png">
                                        <div class="vcentered info-combo" >
                                            
                                            <form name="message-user" id="mform" action="message.php" method="POST">
                                                <input type="hidden" name="fname" value="<?php echo $name?>">
                                                <input type="hidden" name="gender" value="<?php echo $gender?>">
                                                <input type="hidden" name="receiver_id" value="<?php echo $cid?>">
                                                <!--                  <a href="#" style="text-decoration: none;" onclick="document.getElementById('mform').submit();">   -->
                                                <div class="openchat">
                                                    <button style="width: 150%" type="submit"><h3 class="no-margin-bottom name"><?php echo $name; ?></h3>
                                                <h5 >Open Conversation</h5></a></button>
                                            </div>
                                        </form>
                                    </div>
                                    <div class="contacts-add">
                                        <span class="message-time"><?php echo $time ?><sup>PM</sup></span>
                                        
                                        <a href="includes/delete_msgs.inc.php?receiver_id=<?php echo $receiver_id ?>"><button onclick="return confirm('Delete Chat?')"><i class="fa fa-trash-o"></i> </button></a>
...
{
    echo "<p class'success-text' style='color:orange; font-size:20px;text-align: center;'>No Chats!<br> </p>".mysqli_error($conn);
                exit();
}
}
else
